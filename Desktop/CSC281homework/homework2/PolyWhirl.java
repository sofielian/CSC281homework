...
public class PolyWhirl extends JFrame {
  ...
  class Surface extends JPanel {
    ...
    public Surface() {
      ...
      hexagon = new ArrayList<Hex>();
      pentagon = new ArrayList<Pent>();
    ...
    /**
   * This is the Scanner of the coordinates that you will input in the command line
   * 
   * Initialize the scanner
   * Then try a while loop that checks each line
   * Change the first three inputs to an integer and giving them variables to call
   * go through all the shape’s sides in if statements beginning with the triangle
   * In each if statement the command is pointing to the lists created above and adding 
   * the coordinates inputted to the list
   * Put the scanner at the end so it keeps scanning each line
   * Once all the lines are done being scanned, close the scanner
   * Catch is there to finish off the try {} and closes the scanner again
   * Finally finishes the try after catch with a final if the scan is not null
   */
    ...
    public void readAndGenerateShapes() {
	 Scanner scan = new Scanner(System.in);
	     ...
	     int x = scan.nextInt();
             int y = scan.nextInt(); 
	     if(shape == 3){
		  this.triangles.add(new Triangle(x,y));
  		}
                else if(shape == 4){
		  this.quad.add(new Quad(x,y));
		}
		else if(shape == 5){
       		  this.hexagon.add(new Hex(x,y));
		}
		else if(shape == 6){
                  this.pentagon.add(new Pent(x,y));
		}
    ...
    public void drawShapes(Graphics g) {
      ...
      for (Quad q : this.quad) {
        q.draw(g);
      }

      for (Hex h : this.hexagon) {
        h.draw(g);
      }

      for (Pent p : this.pentagon) {
        p.draw(g);
      }
