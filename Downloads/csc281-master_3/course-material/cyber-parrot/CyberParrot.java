...
public class CyberParrot {
    ...
    try {
      ...
      while(fileScanner.hasNextLine()) {
        String line = fileScanner.nextLine();
       	  for (String word : splitByWhitespace(line)){
  ...
  public void speak() {
     ...
     */
 int counter = 0;
  max counter = Random().nextInt(this.wordCount);

	while int counter <= max counter;
  for word in this.wordFrequencies;
	this.wordFrequencies.get(word);
	int counter ++;
